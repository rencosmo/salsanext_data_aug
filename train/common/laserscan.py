...
import common.data_augmentation as data_aug
...
class SemLaserScan(LaserScan):
    ...
    def open_scan_label_dataaug(self, scan_file, label_file):
        ...
        if not any(scan_file.endswith(ext) for ext in self.EXTENSIONS_SCAN):
        ...
        if not isinstance(label_file, str):
            raise TypeError("Filename should be string type, "
                            "but was {type}".format(type=str(type(label_file))))

        # check extension is a laserscan
        if not any(label_file.endswith(ext) for ext in self.EXTENSIONS_LABEL):
